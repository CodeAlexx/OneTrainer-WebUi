...
class ModelType(str, Enum):
    ...
    KANDINSKY_5_VIDEO_PRO = "kandinsky_5_video_pro"
...
class InferenceEngine:
    ...
    def load_model(self, request: LoadModelRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            hf_model_types = [ModelType.KANDINSKY_5, ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO,
            ...
            hf_id_model_types = [ModelType.KANDINSKY_5, ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO,
                                ModelType.QWEN_IMAGE, ModelType.QWEN_IMAGE_EDIT,
                                ModelType.LUMINA, ModelType.LUMINA_2,
                                ModelType.OMNIGEN, ModelType.OMNIGEN_2]
            ...
            large_model_types = [
                ...
                ModelType.KANDINSKY_5, ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO
            ]
    ...
    def _load_single_file(self, model_path: str, model_type: ModelType, dtype):
        ...
        elif model_type in [ModelType.KANDINSKY_5, ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO]:
            return self._load_kandinsky5_single_file(model_path, dtype, video=(model_type in [ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO]))
        else:
    ...
    def _load_qwen_image_single_file(self, model_path: str, dtype, edit_mode: bool = False):
        """Load Qwen-Image model from single file or HuggingFace."""
        ...
        # Try loading from HF cache first (Qwen/Qwen-Image)
        try:
            ...
            pipe.enable_sequential_cpu_offload()
            print("✅ Qwen-Image loaded from HuggingFace cache")
            return pipe
        except Exception as e:
            print(f"Cache load failed: {e}")

        # Try single file loading
        try:
            ...
            pipe.enable_sequential_cpu_offload()
            print("✅ Qwen-Image loaded from single file")
            return pipe
        except Exception as e:
        ...
        try:
            print("Downloading Qwen-Image from HuggingFace...")
            pipe = DiffusionPipeline.from_pretrained(
                "Qwen/Qwen-Image",
                torch_dtype=dtype,
                cache_dir=hf_cache,
            ...
            pipe.enable_sequential_cpu_offload()
            print("✅ Qwen-Image downloaded and loaded")
            return pipe
        except Exception as e:
            print(f"❌ Failed to load Qwen-Image: {e}")
            return None
    ...
    def generate(self, request: GenerateRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            for batch_idx in range(request.batch_count):
                ...
                is_kandinsky = self.model_type in [ModelType.KANDINSKY_5, ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO]
                if not is_kandinsky:
                ...
                # Run inference
                print(f"Running inference with kwargs: {list(kwargs.keys())}")
                result = self.pipeline(**kwargs)

                # Process output - handle different return types
                images = []
                if is_kandinsky:
                    # Kandinsky returns PIL image(s) directly or in result
                    ...
                    print(f"Kandinsky returned {len(images)} images")
                elif hasattr(result, 'images') and result.images:
                    images = list(result.images)
                ...
                if hasattr(result, 'frames') and not images:
                    gen_time = time.time() - start_time
    ...
    def _load_kandinsky5_single_file(self, model_path: str, dtype, video: bool = False):
        ...
        # Check if model_path is local file or needs to use HuggingFace
        ...
        if not Path(model_path).exists():
            ...
            if video:
                ...
                if local_weights.exists() and "pro" in model_path.lower():
                    model_path = str(local_weights)
                    print(f"Using local weights: {model_path}")
                else:
                    # Will use HuggingFace download via kandinsky utils
                    use_hf_download = True
                    print(f"Will download from HuggingFace: {model_path}")
            else:
                # T2I - always use HuggingFace download
                use_hf_download = True
                print(f"Will download from HuggingFace: {model_path}")
        ...
        try:
            ...
            if video:
                ...
                # For Pro models, pass conf_path for correct architecture (4096 dims)
                # The dit_path overrides the checkpoint_path in the config
                conf_path_str = str(config_file) if config_file and config_file.exists() else None
                if conf_path_str:
                    print(f"Using config: {conf_path_str}")
                
                # Component paths - use HF cache IDs for from_pretrained
                # Note: VAE expects directory with config.json, not single safetensors
                vae_path = None  # Let kandinsky download VAE to cache
                text_encoder_path = "Qwen/Qwen2.5-VL-7B-Instruct"  # HF cache
                text_encoder2_path = "openai/clip-vit-large-patch14"  # HF cache
                ...
                # For Pro models, pass conf_path for correct architecture
                conf_path_str = str(config_file) if config_file and config_file.exists() else None
                if conf_path_str:
                    print(f"Using config: {conf_path_str}")
    ...
    def _build_pipeline_kwargs(self, request: GenerateRequest, generator) -> Dict[str, Any]:
        ...
        if self.model_type in [ModelType.KANDINSKY_5, ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO]:
            # Extract seed from generator
            seed = generator.initial_seed() if hasattr(generator, 'initial_seed') else None

            # Kandinsky has specific allowed resolutions
            K5_RESOLUTIONS_512 = [(512, 512), (512, 768), (768, 512)]
            K5_RESOLUTIONS_1024 = [(1024, 1024), (640, 1408), (1408, 640), (768, 1280), (1280, 768), (896, 1152), (1152, 896)]

            # Check and adjust resolution
            width, height = request.width, request.height
            requested_res = (height, width)

            # Determine target resolution set based on requested size
            if max(width, height) > 768:
                allowed = K5_RESOLUTIONS_1024
                target_base = 1024
            else:
                allowed = K5_RESOLUTIONS_512
                target_base = 512

            # Find closest allowed resolution
            if requested_res not in allowed:
                # Find closest match by aspect ratio
                aspect = width / height
                best_match = min(allowed, key=lambda r: abs((r[1] / r[0]) - aspect))
                height, width = best_match
                print(f"⚠️ Adjusted Kandinsky resolution from {request.width}x{request.height} to {width}x{height}")

            # Video-specific params
            is_video = self.model_type in [ModelType.KANDINSKY_5_VIDEO, ModelType.KANDINSKY_5_VIDEO_PRO]
            ...
            kwargs = {
                ...
                "expand_prompts": True,  # Use Qwen to enhance prompts
                "progress": True,
                "scheduler_scale": 10.0 if is_video else 3.0,
            }

            if is_video:
                # time_length is in seconds (frames = time_length * 24 // 4 + 1)
                # Default 5s = 31 frames
                if request.num_frames and request.num_frames > 1:
                    # Convert frames back to seconds: seconds = (frames - 1) * 4 / 24
                    kwargs["time_length"] = max(1, int((request.num_frames - 1) * 4 / 24))
                else:
                    kwargs["time_length"] = 5  # Default 5 seconds

            return kwargs

        # Standard diffusers kwargs
        kwargs = {
            "prompt": request.prompt,
