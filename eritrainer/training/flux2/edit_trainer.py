...
class Flux2EditTrainer(Flux2BaseTrainer):
    ...
    Usage:
        ...
        adapter = create_adapter("lora", rank=32, model_type="flux_klein")
    ...
    def _apply_reference_dropout(self, reference_latents: Tensor) -> Tensor:
        ...
        """
        if not self.training or self.config.reference_dropout <= 0:
    ...
    def forward_transformer(
        ...
        **kwargs,
        ...
        # Pack latents
        packed_latents = self.model.pack_latents(conditioned_latents)
        ...
        output = self.model.transformer(
            ...
            txt_ids=torch.zeros(batch_size, text_embeddings.shape[1], 3, device=self.train_device),
            img_ids=image_ids,
            return_dict=False,
        )
    ...
    def training_step(self, batch: Dict[str, Any]) -> Dict[str, Tensor]:
        ...
        noisy_latents = self.compute_noisy_latents(latents, noise, timesteps)
