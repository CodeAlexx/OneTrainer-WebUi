...
from modules.util.enum.BucketBalancing import BucketBalancing
...
class QuantizationConfig(BaseConfig):
    ...
    def default_values():
        ...
        data.append(("enable_nf4_quantization", False, bool, False))
...
class TrainConfig(BaseConfig):
    ...
    blocks_to_swap: int  # For video models (Wan, HunyuanVideo) - number of transformer blocks to swap for low VRAM
    musubi_blocks_to_swap: int  # Musubi block swap for training - number of transformer blocks to stream CPU↔GPU
    ...
    bucket_quantization: int
    aspect_tolerance: float
    bucket_repeat_small: bool
    bucket_log_dropped: bool
    bucket_preset: str
    bucket_balancing: BucketBalancing
    bucket_min_size: int
    bucket_merge_threshold: int
    bucket_max_per_batch: int
    bucket_custom_aspects: str
    latent_caching: bool
    ...
    epochs: int
    max_steps: int  # If set (> 0), use step-based training instead of epochs
    batch_size: int
    ...
    # Diffusion-4K settings (wavelet-based loss for high-res detail preservation)
    diffusion_4k_enabled: bool
    diffusion_4k_wavelet_loss_weight: float
    diffusion_4k_wavelet_type: str
    ...
    # lycoris (official library integration)
    lycoris_use_tucker: bool
    lycoris_use_scalar: bool
    lycoris_train_norm: bool
    lycoris_rescaled: bool
    lycoris_bypass_mode: bool
    lycoris_dora_wd: bool
    lycoris_factor: int
    lycoris_decompose_both: bool
    lycoris_full_matrix: bool
    lycoris_block_size: int

    # multi-adapter support (stacking multiple LoRA/PEFT adapters)
    adapters: list[dict]  # List of adapter configs for multi-adapter mode
    ...
    def __migration_8(self, data: dict) -> dict:
        ...
        if "transformer" not in migrated_data and migrated_data["model_type"] != "STABLE_CASCADE_1" and migrated_data["model_type"] != "WUERSTCHEN_2":
    ...
    def __migration_9(self, data: dict) -> dict:
        ...
        def replace_dtype(part: str):
            if part in migrated_data and migrated_data[part].get("weight_dtype") == "NONE":
                migrated_data[part]["weight_dtype"] = migrated_data.get("weight_dtype", "FLOAT_16")
        replace_dtype("unet")
        replace_dtype("prior")
        ...
        migrated_data.pop("weight_dtype", None)
    ...
    def default_values() -> 'TrainConfig':
        ...
        data.append(("wandb_project", "onetrainer", str, False))
        data.append(("wandb_entity", "", str, False))
        data.append(("wandb_run_name", "", str, False))
        data.append(("wandb_tags", "", str, False))
        data.append(("wandb_base_url", "http://localhost:8080", str, False))
        ...
        data.append(("blocks_to_swap", 0, int, False))  # 0 = disabled, 32 = recommended for Wan 14B on 24GB
        data.append(("musubi_blocks_to_swap", 0, int, False))  # 0 = auto (half blocks when checkpointing on), or specify exact count
        ...
        data.append(("bucket_quantization", 64, int, False))
        data.append(("aspect_tolerance", 0.15, float, False))
        data.append(("bucket_repeat_small", True, bool, False))
        data.append(("bucket_log_dropped", True, bool, False))
        data.append(("bucket_preset", "default", str, False))
        data.append(("bucket_balancing", BucketBalancing.OFF, BucketBalancing, False))
        data.append(("bucket_min_size", 1, int, False))
        data.append(("bucket_merge_threshold", 0, int, False))
        data.append(("bucket_max_per_batch", 0, int, False))
        data.append(("bucket_custom_aspects", "", str, True))
        ...
        data.append(("max_steps", 0, int, False))  # 0 = use epochs, > 0 = use steps
        ...
        # Diffusion-4K settings (wavelet-based loss for high-res detail preservation)
        data.append(("diffusion_4k_enabled", False, bool, False))
        data.append(("diffusion_4k_wavelet_loss_weight", 1.0, float, False))
        data.append(("diffusion_4k_wavelet_type", "haar", str, False))
        ...
        # lycoris (official library integration)
        data.append(("lycoris_use_tucker", False, bool, False))
        data.append(("lycoris_use_scalar", False, bool, False))
        data.append(("lycoris_train_norm", False, bool, False))
        data.append(("lycoris_rescaled", False, bool, False))
        data.append(("lycoris_bypass_mode", False, bool, False))
        data.append(("lycoris_dora_wd", False, bool, False))
        data.append(("lycoris_factor", -1, int, False))
        data.append(("lycoris_decompose_both", False, bool, False))
        data.append(("lycoris_full_matrix", False, bool, False))
        data.append(("lycoris_block_size", 4, int, False))

        # multi-adapter support
        data.append(("adapters", [], list, False))
