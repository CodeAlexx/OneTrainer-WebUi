...
class BaseQwenSetup(
    ...
    def setup_optimizations(
        ...
        # NOTE: Musubi block swap is INCOMPATIBLE with gradient checkpointing
        # Diffusers' gradient checkpointing uses _gradient_checkpointing_func which
        # bypasses the module's forward hooks, causing device mismatch errors
        # See: SimpleTuner also disables streaming offload when checkpointing is enabled
        blocks_to_swap = getattr(config, 'musubi_blocks_to_swap', 0)

        if config.gradient_checkpointing.enabled() and blocks_to_swap > 0:
            print("Warning: Musubi block swap disabled - incompatible with gradient checkpointing")
            blocks_to_swap = 0

        if blocks_to_swap > 0 and model.transformer is not None and hasattr(model.transformer, 'transformer_blocks'):
            ...
            if model.musubi_manager is not None:
                ...
                print(f"Musubi block swap enabled for Qwen training: {blocks_to_swap} blocks swapped to CPU")
