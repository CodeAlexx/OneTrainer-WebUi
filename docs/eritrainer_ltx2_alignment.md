# EriTrainer LTX2 Alignment Notes

## Scope
Align EriTrainer LTX2 training and caching with OneTrainer and upstream LTX2
behavior while avoiding MGDS.

## Key Differences Identified
- VAE caching path: EriTrainer used a diffusers-style VAE loader; upstream LTX2
  expects ltx_core VideoEncoder/VideoDecoder for video latents.
- Loss space: EriTrainer compared patchified predictions against unpatchified
  targets, which mismatched shapes and training objectives.
- Noise/timestep sampling: EriTrainer used uniform sampling; OneTrainer uses
  shifted logit-normal sampling based on sequence length and supports offset and
  perturbation noise.
- Offload/checkpointing: EriTrainer used staged offload and legacy activation
  offloading; OneTrainer uses per-block checkpointing with a LayerOffloadConductor.

## Fixes Applied
- `eritrainer/models/ltx2.py`: load VAE-only video encoder/decoder from ltx_core
  for video caching compatibility.
- `eritrainer/components/model_setup.py`: LTX2 flow matching now unpatchifies
  predicted flow, uses shifted logit-normal timesteps, and matches OneTrainer
  noise options (offset/perturbation).
- `eritrainer/training/ltx2_checkpointing.py`: OneTrainer-style per-block
  checkpointing with LayerOffloadConductor (no MGDS).
- `eritrainer/core/trainer.py`: when LayerOffloadConductor is active, skip the
...
`venv/bin/python -m eritrainer train --config eritrainer_ltx2_test.yaml --steps 1`
